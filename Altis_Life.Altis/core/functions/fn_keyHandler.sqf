...
switch (_code) do {
	...
	case 19:
	{
		...
		if(_shift && playerSide == west && !isNull cursorTarget && cursorTarget isKindOf "Man" && (isPlayer cursorTarget) && (side cursorTarget != west) && alive cursorTarget && cursorTarget distance player < 3.5 && !(cursorTarget GVAR "Escorting") && !(cursorTarget GVAR "restrained") && speed cursorTarget < 1) then
		{
	...
	case 38: {
		...
		if(_shift && playerSide in [west,independent]) then {
			if(vehicle player != player && (typeOf vehicle player) in ["I_Quadbike_01_F","C_Hatchback_01_sport_F","C_Offroad_01_F","C_SUV_01_F","B_MRAP_01_F","B_Heli_Light_01_F","I_Heli_light_03_unarmed_F","O_Heli_Light_02_unarmed_F","B_Heli_Transport_03_unarmed_F","B_Heli_Transport_01_F"]) then {
