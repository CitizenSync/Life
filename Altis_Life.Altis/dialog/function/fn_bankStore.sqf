#include <macro.h>
/*
	File: fn_bankStore.sqf
	Author: Bryan "Tonic" Boardwine

	Description:
	Gateway copy of fn_vehStoreItem but designed for the safe.
*/
private["_ctrl","_num"];
disableSerialization;
_ctrl = CONTROL_DATA(3503);
_num = ctrlText 3506;

//Error checks
if(!([_num] call TON_fnc_isnumber)) exitWith {hint localize "STR_MISC_WrongNumFormat";};
_num = parseNumber(_num);
if(_num < 1) exitWith {hint localize "STR_Cop_VaultUnder1";};
if(!(EQUAL(_ctrl,"goldcoins"))) exitWith {hint localize "STR_Cop_OnlyGold"};
if(_num > life_inv_goldcoins) exitWith {hint format[localize "STR_Cop_NotEnoughGold",_num];};

//Store it.
if(!([false,_ctrl,_num] call life_fnc_handleInv)) exitWith {hint localize "STR_Cop_CantRemove";};
_bankInfo = life_vaultObj GVAR["vault",0];
life_vaultObj GVAR ["vault",_bankInfo + _num,true];

[life_vaultObj] call life_fnc_bankInventory;