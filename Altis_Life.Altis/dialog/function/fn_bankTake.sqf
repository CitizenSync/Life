#include <macro.h>
/*
	File: fn_vaultTake.sqf
	Author: Bryan "Tonic" Boardwine
	
	Description:
	Gateway to fn_vehTakeItem.sqf but for vault(s).
*/
private["_ctrl","_num","_bankInfo"];
disableSerialization;

if((lbCurSel 3502) == -1) exitWith {hint localize "STR_Civ_SelectItem";};
_ctrl = CONTROL_DATA(3502);
_num = ctrlText 3505;
_bankInfo = life_safeObj GVAR ["vault",0];

//Error checks
if(!([_num] call TON_fnc_isnumber)) exitWith {hint localize "STR_MISC_WrongNumFormat";};
_num = parseNumber(_num);
if(_num < 1) exitWith {hint localize "STR_Cop_VaultUnder1";};
if(!(EQUAL(_ctrl,"goldcoins"))) exitWith {hint localize "STR_Cop_OnlyGold"};
if(_num > _bankInfo) exitWith {hint format[localize "STR_Civ_IsntEnoughGold",_num];};
...
life_vaultObj SVAR ["vault",_bankInfo - _num,TRUE];
